...
@IonicPage()
@Component({
  selector: 'page-pendienteProveedor',
  templateUrl: 'pendienteProveedor.html',
})
...
export class PendienteProveedorPage {
  ...
  ionViewWillEnter() {
    ...
    .then(()=>{
      var query = `?proveedor=${this.usuario.id}&pendientes`;
    ...
    .subscribe(solicitudes => {
      ...
      if(this.util.isEmpty(solicitudes)){
        const toast =this.toastCtrl.create({
          message:"No tienes alquileres pendientes",
          duration:3000
        });
        toast.present();
      }
