...
@IonicPage()
@Component({
  selector: 'page-finalizadoProveedor',
  templateUrl: 'finalizadoProveedor.html',
})
...
export class FinalizadoProveedorPage {
  usuario;
  solicitudes;
  ...
  ionViewWillEnter() {
    ...
    .then(()=>{
      var query = `?proveedor=${this.usuario.id}&finalizado`;
    ...
    .subscribe(solicitudes => {
      ...
      if(this.util.isEmpty(solicitudes)){
        const toast =this.toastCtrl.create({
          message:"No tienes alquileres finalizadas",
          duration:3000
        });
        toast.present();
      }
