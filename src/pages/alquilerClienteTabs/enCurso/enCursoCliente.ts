...
@IonicPage()
@Component({
  selector: 'page-enCursoCliente',
  templateUrl: 'enCursoCliente.html',
})
...
export class EnCursoClientePage {
  usuario;
  solicitudes;
  ...
  ionViewWillEnter() {
    ...
    .then(()=>{
      var query = `?cliente=${this.usuario.id}&enCurso`;
    ...
    .subscribe(solicitudes => {
      ...
      if(this.util.isEmpty(solicitudes)){
        const toast =this.toastCtrl.create({
          message:"No tienes solicitudes en curso",
          duration:3000
        });
        toast.present();
      }
